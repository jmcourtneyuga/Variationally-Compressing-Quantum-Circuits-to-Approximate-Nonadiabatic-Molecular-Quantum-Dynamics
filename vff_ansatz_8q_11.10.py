...
from qiskit.circuit.library.standard_gates import RZGate, RYGate, CXGate, RXGate
...
def ZZZ(qc, qr, qubit_1, qubit_2, qubit_3, theta):
    qc.append(RYGate(np.pi/2), [qr[qubit_3]])
    qc.append(CXGate(), [qr[qubit_2], qr[qubit_1]])
    qc.append(CXGate(), [qr[qubit_3], qr[qubit_2]])
    qc.append(RZGate(theta), [qr[qubit_3]])
    qc.append(CXGate(), [qr[qubit_3], qr[qubit_2]])
    qc.append(CXGate(), [qr[qubit_2], qr[qubit_1]])
    qc.append(RYGate(-np.pi/2), [qr[qubit_3]])
...
def _build_W_operations(thetas_W, n_qubits, num_layers_eigenvector):
    ...
    for layer in range(num_layers_eigenvector):
        ...
        for qubit in range(n_qubits):
            layer_ops.append(("RZ", qubit, thetas_W[idx]))
            idx += 1
      ...
      #  for qubit in range(0, n_qubits-1, 3):
      #      layer_ops.append(("ZZZ", qubit, qubit + 1, qubit + 2, thetas_W[idx]))
      #      idx += 1

     #   for qubit in range(1, n_qubits-1, 3):
     #       layer_ops.append(("ZZZ", qubit, qubit + 1, qubit + 2, thetas_W[idx]))    
     #       idx += 1
...
def build_W_circuit(qc, qr, thetas_W, n_qubits, num_layers_eigenvector):
    ...
    for layer in range(num_layers_eigenvector):
        ...
        for qubit in range(n_qubits):
            qc.append(RZGate(thetas_W[idx]), [qr[qubit]])
            layer_ops.append(("RZ", qubit, thetas_W[idx]))
            idx += 1
    ...
    #    for qubit in range(0, n_qubits-1, 3):
    #        ZZZ(qc, qr, qubit, qubit + 1, qubit + 2, thetas_W[idx])
    #        layer_ops.append(("ZZZ", qubit, qubit + 1, qubit + 2, thetas_W[idx]))
    #        idx += 1
...
def build_W_dagger_circuit_with_ops(qc, qr, eigenvector_ops, n_qubits):
    for layer_ops in reversed(eigenvector_ops):
        for op in reversed(layer_ops):
            if op[0] == "RZ":
                _, qubit, theta = op
...
def calculate_parameter_counts(n_qubits, num_layers_eigenvector, num_layers_diagonal):
    ...
    d_params_per_layer = n_qubits + n_qubits//2 +  (n_qubits - 1)//2
...
def get_full_vff_quantum_circuit(thetas, num_layers_eigenvector, num_layers_diagonal, n_qubits, 
                              w_shift_type=None, d_shift_type=None, shift_param_idx=None):
    ...
    # Apply parameter shifts if requested
    if shift_param_idx is not None:
        if w_shift_type is not None and shift_param_idx < total_w_params:
            if w_shift_type in ['I+', 'I-']:  # Shift in first W
                if w_shift_type == 'I+':
                    thetas_W_forward[shift_param_idx] += np.pi/2
                elif w_shift_type == 'I-':
                    thetas_W_forward[shift_param_idx] -= np.pi/2
            elif w_shift_type in ['O+', 'O-']:  # Shift in W†
                if w_shift_type == 'O+':
                    thetas_W_dagger[shift_param_idx] += np.pi/2
                elif w_shift_type == 'O-':
                    thetas_W_dagger[shift_param_idx] -= np.pi/2

        if d_shift_type is not None and shift_param_idx >= total_w_params:
            d_idx = shift_param_idx - total_w_params
            if d_idx < len(thetas_D):
                if d_shift_type == 'I+':
                    thetas_D[d_idx] += np.pi/2
                elif d_shift_type == 'I-':
                    thetas_D[d_idx] -= np.pi/2
    ...
    if w_shift_type in ['O+', 'O-']:
        eigenvector_ops_dagger, _ = _build_W_operations(thetas_W_dagger, n_qubits, num_layers_eigenvector)
        build_W_dagger_circuit_with_ops(qc, qr, eigenvector_ops_dagger, n_qubits)
    else:
        # Use the same operations as forward W
